...
export default function EditExamPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setError(err?.data?.message || "Failed to load exam data");
      } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      router.push(`/dashboard/teacher/exams/${examId}?updated=true`);
    } catch (err) {
      setError(err?.data?.message || "Failed to update exam");
    } finally {
    ...
    <DashboardLayout>
      <Section className="flex flex-col pt-8 pb-12">
        ...
        <div className="flex flex-col items-center justify-center">
          <h1 className="mb-8 font-semibold text-2xl text-center">Edit Exam</h1>
